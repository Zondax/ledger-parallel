...
{
    CHECK_INPUT()
    CHECK_ERROR(_readAssetIdOf_V16(c, &v->assetid1))
    CHECK_ERROR(_readAssetIdOf_V16(c, &v->assetid2))
    return parser_ok;
}
...
{
    CHECK_INPUT()
    CHECK_ERROR(_readCurrencyId_V16(c, &v->id))
...
{
    CHECK_INPUT()
    CHECK_ERROR(_readRate_V16(c, &v->baseRate))
    return parser_ok;
}
...
parser_error_t _readInterestRateModel_V16(parser_context_t* c, pd_InterestRateModel_V16_t* v)
{
    ...
    switch (v->value) {
    case 0:
        CHECK_ERROR(_readJumpModel_V16(c, &v->jump))
        break;
    case 1:
        CHECK_ERROR(_readCurveModel_V16(c, &v->curve))
        break;
    default:
        return parser_unexpected_value;
    }
...
parser_error_t _readJumpModel_V16(parser_context_t* c, pd_JumpModel_V16_t* v)
{
    CHECK_INPUT()
    CHECK_ERROR(_readRate_V16(c, &v->baseRate))
    CHECK_ERROR(_readRate_V16(c, &v->jumpRate))
    CHECK_ERROR(_readRate_V16(c, &v->fullRate))
    CHECK_ERROR(_readRatio_V16(c, &v->jumpUtilization))
    return parser_ok;
}
...
{
    CHECK_INPUT()
    CHECK_ERROR(_readRatio_V16(c, &v->collateralFactor))
    CHECK_ERROR(_readRatio_V16(c, &v->liquidationThreshold))
    CHECK_ERROR(_readRatio_V16(c, &v->reserveFactor))
    CHECK_ERROR(_readRatio_V16(c, &v->closeFactor))
    CHECK_ERROR(_readRate_V16(c, &v->liquidateIncentive))
    CHECK_ERROR(_readRatio_V16(c, &v->liquidateIncentiveReservedFactor))
    CHECK_ERROR(_readInterestRateModel_V16(c, &v->rateModel))
    CHECK_ERROR(_readMarketState_V16(c, &v->state))
    CHECK_ERROR(_readBalance(c, &v->supplyCap))
    CHECK_ERROR(_readBalance(c, &v->borrowCap))
    CHECK_ERROR(_readCurrencyId_V16(c, &v->ptokenId))
    return parser_ok;
}
...
{
    CHECK_INPUT()
    CHECK_ERROR(_readAccountId_V16(c, &v->stash))
    CHECK_ERROR(_readCompactu128(c, &v->total))
    CHECK_ERROR(_readCompactu128(c, &v->active))
    CHECK_ERROR(_readVecUnlockChunk_V16(c, &v->unlocking))
    CHECK_ERROR(_readVecEraIndex_V16(c, &v->claimedRewards))
    return parser_ok;
}
...
parser_error_t _readOptionContributionStrategy_V16(parser_context_t* c, pd_OptionContributionStrategy_V16_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readContributionStrategy_V16(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionRate_V16(parser_context_t* c, pd_OptionRate_V16_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readRate_V16(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionRatio_V16(parser_context_t* c, pd_OptionRatio_V16_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readRatio_V16(c, &v->contained))
    }
    return parser_ok;
}
    ...
    uint8_t* pageCount)
{
    ...
    // First measure number of pages
    uint8_t pages[2] = { 0 };
    CHECK_ERROR(_toStringAssetIdOf_V16(&v->assetid1, outValue, outValueLen, 0, &pages[0]))
    CHECK_ERROR(_toStringAssetIdOf_V16(&v->assetid2, outValue, outValueLen, 0, &pages[1]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringAssetIdOf_V16(&v->assetid1, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];

    if (pageIdx < pages[1]) {
        CHECK_ERROR(_toStringAssetIdOf_V16(&v->assetid2, outValue, outValueLen, pageIdx, &pages[1]))
        return parser_ok;
    }
    ...
    uint8_t* pageCount)
{
    ...
    // First measure number of pages
    uint8_t pages[8] = { 0 };
    CHECK_ERROR(_toStringCurrencyId_V16(&v->id, outValue, outValueLen, 0, &pages[0]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringCurrencyId_V16(&v->id, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];
...
{
    CLEAN_AND_CHECK()
    CHECK_ERROR(_toStringRate_V16(&v->baseRate, outValue, outValueLen, pageIdx, pageCount))
    return parser_ok;
}
    ...
    uint8_t* pageCount)
{
    ...
    switch (v->value) {
    case 0:
        CHECK_ERROR(_toStringJumpModel_V16(&v->jump, outValue, outValueLen, pageIdx, pageCount))
        break;
    case 1:
        CHECK_ERROR(_toStringCurveModel_V16(&v->curve, outValue, outValueLen, pageIdx, pageCount))
        break;
    default:
        return parser_unexpected_value;
    }
    ...
    uint8_t* pageCount)
{
    ...
    // First measure number of pages
    uint8_t pages[4] = { 0 };
    CHECK_ERROR(_toStringRate_V16(&v->baseRate, outValue, outValueLen, 0, &pages[0]))
    CHECK_ERROR(_toStringRate_V16(&v->jumpRate, outValue, outValueLen, 0, &pages[1]))
    CHECK_ERROR(_toStringRate_V16(&v->fullRate, outValue, outValueLen, 0, &pages[2]))
    CHECK_ERROR(_toStringRatio_V16(&v->jumpUtilization, outValue, outValueLen, 0, &pages[3]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringRate_V16(&v->baseRate, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];

    if (pageIdx < pages[1]) {
        CHECK_ERROR(_toStringRate_V16(&v->jumpRate, outValue, outValueLen, pageIdx, &pages[1]))
        return parser_ok;
    }
    pageIdx -= pages[1];

    if (pageIdx < pages[2]) {
        CHECK_ERROR(_toStringRate_V16(&v->fullRate, outValue, outValueLen, pageIdx, &pages[2]))
        return parser_ok;
    }
    pageIdx -= pages[2];

    if (pageIdx < pages[3]) {
        CHECK_ERROR(_toStringRatio_V16(&v->jumpUtilization, outValue, outValueLen, pageIdx, &pages[3]))
        return parser_ok;
    }
    ...
    uint8_t* pageCount)
{
    ...
    // First measure number of pages
    uint8_t pages[11] = { 0 };
    CHECK_ERROR(_toStringRatio_V16(&v->collateralFactor, outValue, outValueLen, 0, &pages[0]))
    CHECK_ERROR(_toStringRatio_V16(&v->liquidationThreshold, outValue, outValueLen, 0, &pages[1]))
    CHECK_ERROR(_toStringRatio_V16(&v->reserveFactor, outValue, outValueLen, 0, &pages[2]))
    CHECK_ERROR(_toStringRatio_V16(&v->closeFactor, outValue, outValueLen, 0, &pages[3]))
    CHECK_ERROR(_toStringRate_V16(&v->liquidateIncentive, outValue, outValueLen, 0, &pages[4]))
    CHECK_ERROR(_toStringRatio_V16(&v->liquidateIncentiveReservedFactor, outValue, outValueLen, 0, &pages[5]))
    CHECK_ERROR(_toStringInterestRateModel_V16(&v->rateModel, outValue, outValueLen, 0, &pages[6]))
    CHECK_ERROR(_toStringMarketState_V16(&v->state, outValue, outValueLen, 0, &pages[7]))
    ...
    CHECK_ERROR(_toStringCurrencyId_V16(&v->ptokenId, outValue, outValueLen, 0, &pages[10]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringRatio_V16(&v->collateralFactor, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];

    if (pageIdx < pages[1]) {
        CHECK_ERROR(_toStringRatio_V16(&v->liquidationThreshold, outValue, outValueLen, pageIdx, &pages[1]))
        return parser_ok;
    }
    pageIdx -= pages[1];

    if (pageIdx < pages[2]) {
        CHECK_ERROR(_toStringRatio_V16(&v->reserveFactor, outValue, outValueLen, pageIdx, &pages[2]))
        return parser_ok;
    }
    pageIdx -= pages[2];

    if (pageIdx < pages[3]) {
        CHECK_ERROR(_toStringRatio_V16(&v->closeFactor, outValue, outValueLen, pageIdx, &pages[3]))
        return parser_ok;
    }
    pageIdx -= pages[3];

    if (pageIdx < pages[4]) {
        CHECK_ERROR(_toStringRate_V16(&v->liquidateIncentive, outValue, outValueLen, pageIdx, &pages[4]))
        return parser_ok;
    }
    pageIdx -= pages[4];

    if (pageIdx < pages[5]) {
        CHECK_ERROR(_toStringRatio_V16(&v->liquidateIncentiveReservedFactor, outValue, outValueLen, pageIdx, &pages[5]))
        return parser_ok;
    }
    pageIdx -= pages[5];

    if (pageIdx < pages[6]) {
        CHECK_ERROR(_toStringInterestRateModel_V16(&v->rateModel, outValue, outValueLen, pageIdx, &pages[6]))
        return parser_ok;
    }
    pageIdx -= pages[6];

    if (pageIdx < pages[7]) {
        CHECK_ERROR(_toStringMarketState_V16(&v->state, outValue, outValueLen, pageIdx, &pages[7]))
        return parser_ok;
    }
    pageIdx -= pages[7];
    ...
    if (pageIdx < pages[10]) {
        CHECK_ERROR(_toStringCurrencyId_V16(&v->ptokenId, outValue, outValueLen, pageIdx, &pages[10]))
        return parser_ok;
    }
    ...
    uint8_t* pageCount)
{
    ...
    // First measure number of pages
    uint8_t pages[5] = { 0 };
    CHECK_ERROR(_toStringAccountId_V16(&v->stash, outValue, outValueLen, 0, &pages[0]))
    ...
    CHECK_ERROR(_toStringVecUnlockChunk_V16(&v->unlocking, outValue, outValueLen, 0, &pages[3]))
    CHECK_ERROR(_toStringVecEraIndex_V16(&v->claimedRewards, outValue, outValueLen, 0, &pages[4]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringAccountId_V16(&v->stash, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];
    ...
    if (pageIdx < pages[3]) {
        CHECK_ERROR(_toStringVecUnlockChunk_V16(&v->unlocking, outValue, outValueLen, pageIdx, &pages[3]))
        return parser_ok;
    }
    pageIdx -= pages[3];

    if (pageIdx < pages[4]) {
        CHECK_ERROR(_toStringVecEraIndex_V16(&v->claimedRewards, outValue, outValueLen, pageIdx, &pages[4]))
        return parser_ok;
    }
...
parser_error_t _toStringVecAssetIdOf_V16(
    ...
    uint8_t* pageCount)
{
    GEN_DEF_TOSTRING_VECTOR(AssetIdOf_V16);
}
...
parser_error_t _toStringVecEraIndex_V16(
    ...
    uint8_t* pageCount)
{
    GEN_DEF_TOSTRING_VECTOR(EraIndex_V16);
}
...
parser_error_t _toStringVecUnlockChunk_V16(
    ...
    uint8_t* pageCount)
{
    GEN_DEF_TOSTRING_VECTOR(UnlockChunk_V16);
}
...
parser_error_t _toStringVecVestingScheduleOf_V16(
    ...
    uint8_t* pageCount)
{
    GEN_DEF_TOSTRING_VECTOR(VestingScheduleOf_V16);
}
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringContributionStrategy_V16(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringRatio_V16(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
